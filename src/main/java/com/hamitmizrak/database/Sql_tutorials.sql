...
SELECT * FROM cars.register where email_address='eroot'

-- UPDATE
-- update database.table  set attributes="içerik"
UPDATE `cars`.`register` SET `nick_name`="nick", `email_address`="email@gmail.com", `password`="123456", `roles`="user", `remaining_number`=5, `is_passive`=1 WHERE (`id` = '1');
